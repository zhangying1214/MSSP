function S = csp(I,lambda,sigma,sharpness,maxIter,E,beta)
...
%
    ...
    dec=2.0;
    for iter = 1:maxIter
        [wx, wy] = computeTextureWeights(x, sigma_iter, sharpness,E,beta);
...
function [retx, rety] = computeTextureWeights(fin ,sigma,sharpness,E,beta)
   ...
   wtbx = max(E*beta+sum(abs(gfx),3)/size(fin,3),vareps).^(-1);
   wtby = max(E*beta+sum(abs(gfy),3)/size(fin,3),vareps).^(-1); 
   retx = wtbx.*wto;
   rety = wtby.*wto;
...
% function ret1 = conv2_sep(im, sigma)
%   ksize = bitor(round(5*sigma),1);
%   g = fspecial('gaussian', [1,ksize], sigma);
%   ret1 = conv2(im,g,'same');
%   ret1 = conv2(ret1,g','same');
% end
